...
/**
 * Fenêtre permettant d'ajouter un véhicule au garage.
 */
public class FenetreAjoutVehicule extends Stage {
	...
	/**
	 * Crée la fenêtre pour ajouter un véhicule.
	 */
	 ...
	 */
	public void init(){
		this.listeTypeVehi.getSelectionModel().clearSelection();
	...
	/**
	 * Traite les événements sur le bouton ok et sur le bouton annuler.
	 * @param e
	 */
	private void gererBoutons(ActionEvent e) {
		if (e.getSource()==bnOK) {
			...
			switch(listeTypeVehi.getValue()) {
				case "Camionnette" :
					Principale.ajouterVehicule(new Camionnette(vMax, capac, dPieton, matric, carbu, consoCarbu, bPA, bPB));
					break;
				case "Quad" :
					Principale.ajouterVehicule(new Quad(vMax, capac, dPieton, matric, carbu, consoCarbu, bPA, bPB));
					break;
				case "Staby" :
					Principale.ajouterVehicule(new Staby(vMax, capac, dPieton, matric, carbu, consoCarbu, bPA, bPB));
					break;
				default :
					break;
			}
	...
	/**
	 * Gère les zones de saisie TextField en cas d'absence de saisie, elle désactive le bouton "OK" pour ajouter les informations d'un véhicule.
	 * @param e
	 */
